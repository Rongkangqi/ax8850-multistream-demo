...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        name=name[:-3];p=None;started=time.monotonic();sent=0;reason="unknown"
        try:
            ...
            args=['ffmpeg','-hide_banner','-nostdin','-loglevel','warning',
                  ...
                  '-i',streams[name],'-map','0:v:0','-an','-c:v','copy',
            ...
            upstream_rc=p.poll() if p is not None else None
            if p is not None:
                stop_process(p)
                if p.stdout:p.stdout.close()
                with lock:children.discard(p)
            slots.release()
            logging.info('Preview closed: %s reason=%s bytes=%s duration=%.1f upstream_rc=%s',name,reason,sent,time.monotonic()-started,upstream_rc)
...
def main():
    ...
    parser.add_argument('--config',type=Path,default=ROOT/'run/config.json')
    args=parser.parse_args()
    config=json.loads(args.config.read_text())
    def loopback(uri):
        u=urlsplit(uri)
        if u.scheme!='rtsp':raise ValueError('Preview requires RTSP outputs')
        return u._replace(netloc='127.0.0.1:'+str(u.port or 554)).geturl()
    streams={c['name']:loopback(c['output']) for c in config['channels']}
    streams['overview']=loopback(config['overview'])
